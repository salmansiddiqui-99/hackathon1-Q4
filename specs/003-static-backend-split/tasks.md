...
- [ ] T001 Verify Docusaurus configuration in `textbook/docusaurus.config.js` has `baseUrl: "/hackathon1-Q4/"` and `siteUrl: "https://salmansiddiqui-99.github.io"`
- [ ] T002 Verify FastAPI backend dependencies in `backend/requirements.txt` include FastAPI, Uvicorn, Pydantic, CORS middleware
- [ ] T003 [P] Create `.env` template files: `backend/.env.example` and `textbook/.env.example` documenting all required variables
- [ ] T004 [P] Initialize Git workflow documentation: Create `.github/DEPLOYMENT.md` with setup steps for both frontend and backend
...
- [ ] T005 Implement CORS middleware in `backend/src/config.py` with explicit origin allowlist (GitHub Pages domain + localhost for dev)
- [ ] T006 [P] Create API request/response models in `backend/src/models/requests.py` for ChatbotQuery, HealthCheckResponse, ErrorResponse entities
- [ ] T007 [P] Implement error handling framework in `backend/src/errors.py` with custom exception classes for BACKEND_UNAVAILABLE, TIMEOUT, INTERNAL_ERROR, etc.
- [ ] T008 Create structured logging configuration in `backend/src/logging_config.py` (JSON format for API logs)
- [ ] T009 [P] Set up environment variable validation in `backend/src/config.py` using Pydantic BaseSettings
- [ ] T010 Create FastAPI main application in `backend/src/main.py` with CORS middleware and basic route structure
- [ ] T011 Create API router initialization in `backend/src/api/__init__.py` for modular endpoint registration
- [ ] T012 [P] Create static asset configuration in `textbook/static/js/api-url-config.js` that detects environment and sets API endpoints
...
- [ ] T013 [P] [US1] Create build output validation test in `textbook/tests/test_build_assets.js` that verifies:
...
- [ ] T014 [P] [US1] Create Docusaurus build test in `textbook/tests/test_docusaurus_build.js` that:
...
- [ ] T015 [US1] Audit all asset references in `textbook/src/components/` for absolute paths and convert to `useBaseUrl()` calls or import statements
- [ ] T016 [P] [US1] Update all image imports in `textbook/src/css/custom.css` to use relative paths (e.g., `url('../static/img/logo.svg')`)
- [ ] T017 [P] [US1] Verify all static assets in `textbook/static/img/` are correctly configured in `textbook/docusaurus.config.js`
- [ ] T018 [US1] Add explicit baseUrl configuration verification in build output: ensure `build/img/` and `build/assets/` directories match expected structure
- [ ] T019 [US1] Run `npm run build` locally and verify output shows:
...
- [ ] T020 [P] [US2] Create health endpoint contract test in `backend/tests/contract/test_health.py` that verifies:
...
- [ ] T021 [P] [US2] Create chatbot endpoint contract test in `backend/tests/contract/test_chatbot_api.py` that verifies:
...
- [ ] T022 [P] [US2] Create CORS middleware validation test in `backend/tests/integration/test_cors.py` that verifies:
...
- [ ] T023 [P] [US2] Implement `/api/ready` endpoint in `backend/src/api/health.py`:
  - Route: GET /api/ready
...
- [ ] T024 [P] [US2] Implement `/api/chatbot/query` endpoint skeleton in `backend/src/api/chatbot.py`:
  ...
  - Registered in `backend/src/main.py` with router prefix `/api`
- [ ] T025 [US2] Verify CORS middleware in `backend/src/config.py` is properly registered with FastAPI app:
...
- [ ] T026 [US2] Test health check endpoint locally: `curl http://localhost:8000/api/ready` returns 200 OK
- [ ] T027 [US2] Test CORS headers locally with curl:
...
- [ ] T028 [P] [US3] Create health check integration test in `textbook/tests/integration/test_health_check.js` that verifies:
...
- [ ] T029 [P] [US3] Create error message UI test in `textbook/tests/integration/test_chatbot_error_ui.js` that verifies:
...
- [ ] T030 [P] [US3] Implement health check function in `textbook/src/components/ChatbotWidget.jsx`:
...
- [ ] T031 [P] [US3] Implement useEffect hook in `textbook/src/components/ChatbotWidget.jsx`:
...
- [ ] T032 [US3] Implement retry function in `textbook/src/components/ChatbotWidget.jsx`:
...
- [ ] T033 [US3] Add error UI display logic in `textbook/src/components/ChatbotWidget.jsx`:
...
- [ ] T034 [US3] Update ChatbotWidget input/send button state based on health check:
...
- [ ] T035 [P] [US4] Create credential leak detection test in `textbook/tests/security/test_no_credentials.js` that:
...
- [ ] T036 [P] [US4] Create network traffic inspection test that verifies:
...
- [ ] T037 [US4] Audit `textbook/static/js/api-url-config.js` for hardcoded credentials:
...
- [ ] T038 [P] [US4] Audit `textbook/src/components/ChatbotWidget.jsx` for credential exposure:
...
- [ ] T039 [P] [US4] Audit `.env` files for credential presence:
...
- [ ] T040 [US4] Document credential handling in `specs/003-static-backend-split/security-checklist.md`:
...
- [ ] T041 [P] [US5] Create error message test in `textbook/tests/integration/test_error_messages.js` that verifies:
...
- [ ] T042 [P] [US5] Create error logging test in `backend/tests/integration/test_error_logging.py` that verifies:
...
- [ ] T043 [US5] Implement error mapping function in `textbook/src/components/ChatbotWidget.jsx`:
...
- [ ] T044 [P] [US5] Update fetch error handling in `textbook/src/components/ChatbotWidget.jsx`:
...
- [ ] T045 [US5] Implement error state UI display in `textbook/src/components/ChatbotWidget.jsx`:
...
- [ ] T046 [US5] Implement empty context handling in `backend/src/api/chatbot.py`:
...
- [ ] T047 [US5] Update backend error responses in `backend/src/api/chatbot.py`:
...
- [ ] T048 [P] Integration test: Full flow with backend available - Create `backend/tests/integration/test_e2e_backend_available.py`:
...
- [ ] T049 [P] Integration test: Backend unavailability scenario - Create `backend/tests/integration/test_e2e_backend_unavailable.py`:
...
- [ ] T050 [P] Verify API contract adherence - Create `backend/tests/integration/test_api_contract.py`:
...
- [ ] T052 Run local build validation:
...
- [ ] T055 [P] Production acceptance tests:
...
- [ ] T056 [P] Backend failure scenario on production:
...
- [ ] T057 Acceptance criteria verification:
